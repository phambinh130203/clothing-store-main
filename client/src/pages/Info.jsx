...
const Info = () => {
  ...
  const handleUpdateInfo = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.patch("http://localhost:3003/user/update", user, {
        headers: {
  ...
  const handleRemoveAddress = async (idAddress) => {
    try {
      const res = await axios.delete("http://localhost:3003/address/delete", {
        data: {
          _id: idAddress,
        },
        headers: {
    ...
    <Layout>
      <div className="flex gap-x-12 mx-24 my-10">
        ...
        <div className="w-full">
          <h1 className="font-bold text-3xl text-#3C4242 mb-5">Thông tin của bạn</h1>
          <p className="font-bold text-xl text-#3C4242">Thông tin liên hệ</p>
          <form action="">
            ...
            <div className="py-5 border-b flex justify-between">
              <p className="font-semibold text-lg text-#807D7E">Mật khẩu</p>
              <div className="font-semibold text-lg flex justify-between">
                <Link to={"/newpassword"}>Sửa</Link>
              </div>
            </div>
          </form>
          <div>
            <div className="flex justify-between text-#3C4242 my-7">
              <h2 className="font-bold text-2xl">Địa chỉ</h2>
              <Link
                onClick={() => setCur("Add")}
